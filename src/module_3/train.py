...
from functions import three_way_split_time, get_best_treshold, pick_features_for_splits, logit_significance
...
def feature_label_split(df: pd.DataFrame) -> tuple:
    ...
    X_train, X_val, _, Y_train, Y_val, _ = three_way_split_time(df[features], df['outcome'])
...
def select_features(df: pd.DataFrame) -> list:
    ...
    y_val_pred = np.array([1 if p > best_threshold else 0 for p in model.predict_proba(X_val)[:, 1]])
...
def train_model(df: pd.DataFrame, output_path: str) -> dict:
    ...
    X_train, X_val, Y_train, Y_val = feature_label_split(df)
    X_train, X_val, _ = pick_features_for_splits(X_train, X_val, X_val, selected_features)
